...
def send_daily_reminders():
    ...
    for habit in habits:
        ...
        reminder_time = (datetime.combine(current_date, habit_time) - timedelta(minutes=5)).time()
        ...
        if reminder_time <= current_time <= habit_time:
            ...
            if result is None:
                print(f"Не удалось отправить напоминание для привычки {habit.id}: нет tg_chat_id")
